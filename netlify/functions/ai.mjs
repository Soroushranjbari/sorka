...
const AI_MODELS = (process.env.AI_MODELS ||
  'openrouter/free,qwen/qwen3.8-27b:free,google/gemma-4-31b-it:free,nvidia/nemotron-3-super-120b-a12b:free,thinkingmachines/inkling:free')
...
function parseDraft(raw, catalog) {
  ...
  let txt = String(raw).replace(/^```(?:json)?/i, '').replace(/```$/, '').trim();
  let d;
  try { d = JSON.parse(txt); } catch {
    // Truncated output (finish_reason=length): close any open brackets/quotes
    // so the leading, already-complete exercises survive. A repaired draft is
    // still coach-reviewed, so a slightly shorter list beats a hard failure.
    ...
    // Drop a trailing incomplete exercise object, then close the structure.
    txt = txt.replace(/,\s*\{[^{}]*$/, '');
    txt += inStr ? '"' : '';
...
async function buildContext(st, acct, catalog) {
  const L = [];
  ...
  if (evs.length) line(L, `CALENDAR (${evs.length}): ` + evs.slice(-25).map((e) => `day${e.day} ${e.start}:00 "${e.label || ''}" for ${e.client} [${e.att || 'unmarked'}]`).join(' | '));
  const tps = Array.isArray(d.TEMPLATES) ? d.TEMPLATES : [];
  if (tps.length) line(L, `TEMPLATES (${tps.length}): ` + tps.slice(0, 10).map((t) => `"${t.n}" (${t.days}, ${t.dur}, ${(t.exs || []).length}ex)`).join(' | '));
  if (foods.length) line(L, `FOOD LIBRARY (${foods.length}): ` + foods.slice(0, 60).map((f) => `${f.n} ${f.kcal}kcal/${f.p}p`).join(' | '));
...
async function chat(req, st) {
  ...
  return withLock(`ai:${acct.id}`, async () => {
    ...
    const out = await callModel(messages, (raw) => String(raw || '').trim() || null);
    if (!out) return j(502, { ok: false, error: 'ai-upstream' });
