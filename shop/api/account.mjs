...
//   POST /shop/api/account/login   {email,password} -> {ok,token,coach,billing,quota}
//   GET  /shop/api/account/session (Bearer)         -> {ok,coach,access,billing,quota}
...
import { readJsonCapped, badJson, rateLimit, ipOf, tooMany, secure } from '../../netlify/lib/guard.mjs';
...
export default async (req) => {
  ...
  if (req.method === 'POST' && action === 'login') {
    ...
    const { data: body, bad } = await readJsonCapped(req, 10_000);
  ...
  if (req.method === 'GET' && (action === 'session' || action === 'orders')) {
    ...
    if (action === 'session') {
      ...
      return secure(j(200, { ok: true, coach: me.coach, access: me.access, billing: bill && bill.billing, quota: bill && bill.quota }));
    }
